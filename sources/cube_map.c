...
/*   Updated: 2023/10/24 16:31:15 by smagdela         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool is_zero_sauronded(t_map *map, int i, int j)
{
	if (i == 0 || map->cube_map[i + 1] == NULL)
		...
		return (false);
	else if (map->cube_map[i - 1][j] == ' ' || map->cube_map[i + 1][j] == ' ' || map->cube_map[i][j - 1] == ' ' || map->cube_map[i][j + 1] == ' ')
		return (false);
	return (true);
}
...
static bool conditions_map(int i, int j, t_map *map, int condition)
{
	if (condition == 1)
	{
		if (map->cube_map[i][j] != '1' && map->cube_map[i][j] != '0' && map->cube_map[i][j] != 'N' && map->cube_map[i][j] != 'E' && map->cube_map[i][j] != 'S' && map->cube_map[i][j] != 'W' && map->cube_map[i][j] != ' ')
			return (false);
	}
	if (condition == 2)
	{
		if ((map->cube_map[i][j] == '0' || map->cube_map[i][j] == 'N' || map->cube_map[i][j] == 'S' || map->cube_map[i][j] == 'W' || map->cube_map[i][j] == 'E') && !is_zero_sauronded(map, i, j))
			return (false);
	}
	if (condition == 3)
	{
		if (map->cube_map[i][j] == 'N' || map->cube_map[i][j] == 'E' || map->cube_map[i][j] == 'S' || map->cube_map[i][j] == 'W')
			return (false);
	}
	return (true);
}
